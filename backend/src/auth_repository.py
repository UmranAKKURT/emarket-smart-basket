...
class AuthRepository:
    ...
    def create_user(
        ...
        with self.db_helper.get_connection() as connection:
            cursor = connection.execute(
                ...
                VALUES (?, ?, ?, ?, ?);
                """,
                (email, password_hash, role, int(is_active), datetime.now(timezone.utc).isoformat(timespec="seconds")),
            )
            connection.commit()
    ...
    def record_failed_login(self, user_id: int, failed_attempts: int, locked_until: str | None) -> None:
        with self.db_helper.get_connection() as connection:
            connection.execute(
        ...
        with self.db_helper.get_connection() as connection:
            cursor = connection.execute(
                ...
                (
                    ...
                    values["created_at"], values["expires_at"], values.get("last_seen_at"),
                    values.get("user_agent"), values.get("ip_address"),
                ),
            )
            connection.commit()
            return int(cursor.lastrowid)
    ...
    def revoke_session(self, token_hash: str, revoked_at: str) -> None:
        with self.db_helper.get_connection() as connection:
            connection.execute("UPDATE admin_sessions SET revoked_at = ? WHERE token_hash = ?;", (revoked_at, token_hash))
            connection.commit()

    def revoke_all_user_sessions(self, user_id: int, revoked_at: str) -> None:
        with self.db_helper.get_connection() as connection:
            connection.execute("UPDATE admin_sessions SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL;", (revoked_at, user_id))
            connection.commit()

    def touch_session(self, token_hash: str, last_seen_at: str) -> None:
        with self.db_helper.get_connection() as connection:
            connection.execute("UPDATE admin_sessions SET last_seen_at = ? WHERE token_hash = ?;", (last_seen_at, token_hash))
            connection.commit()

    def delete_expired_sessions(self, now: str) -> int:
        with self.db_helper.get_connection() as connection:
            cursor = connection.execute("DELETE FROM admin_sessions WHERE expires_at <= ?;", (now,))
            connection.commit()
